...
// GET livreurs en attente de vérification
router.get('/livreurs/en-attente', authMiddleware, checkRole('admin'), async (req, res) => {
  try {
    const { limit = 50, offset = 0 } = parseLimitOffset(req.query.limit, req.query.offset);
    
    const result = await db.query(
      `SELECT u.id, u.email, u.nom_complet, u.telephone, u.role, u.statut, u.date_creation,
    ...
    // Récupérer les documents pour chaque livreur
    const livreurs = await Promise.all(
      result.rows.map(async (row) => {
        let documents = [];
        if (row.livreur_id) {
          const docsResult = await db.query(
            `SELECT id, type_document, chemin_fichier, date_upload
             FROM documents_livreurs
             WHERE livreur_id = $1
    ...
    console.error('Error fetching pending livreurs:', error);
    return res.status(500).json({ 
      success: false, 
      message: 'Erreur lors de la récupération des livreurs en attente',
      details: error.message 
    });
  }
});

// PUT - Accepter ou rejeter un livreur
router.put('/livreurs/:livreurId/verifier', authMiddleware, checkRole('admin'), async (req, res) => {
  const client = await db.connect();
  try {
    const { livreurId } = req.params;
    const { action, raison_rejet } = req.body; // action: 'accepter' ou 'rejeter'

    if (!action || !['accepter', 'rejeter'].includes(action)) {
      return res.status(400).json({ 
        success: false, 
        message: 'Action invalide. Doit être "accepter" ou "rejeter"' 
      });
    }
    ...
    const message = action === 'accepter' 
      ? 'Livreur accepté et compte activé'
      : 'Livreur rejeté';

    return res.json({ 
      success: true, 
      message: message,
      data: { livreurId, newStatus }
    });
  } catch (error) {
    await client.query('ROLLBACK').catch(() => {});
    console.error('Error verifying livreur:', error);
    return res.status(500).json({ 
      success: false, 
...
// GET - Récupérer les détails complètes d'un livreur en attente
router.get('/livreurs/:livreurId/details', authMiddleware, checkRole('admin'), async (req, res) => {
  try {
    const { livreurId } = req.params;
    ...
    return res.status(500).json({ 
      success: false, 
      message: 'Erreur lors de la récupération des détails du livreur',
      details: error.message 
    });
  }
});
